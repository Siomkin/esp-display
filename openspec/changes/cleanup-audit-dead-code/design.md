...
1. **Topic table** (item 7). A static array of `{topic, float *value, bool *valid, label, decimals, unit}` (a per-row format string is impossible: `ESP_LOGI` concatenates its format at compile time, so the log line is built as `"%s: %.*f%s"`) covers the four float topics, pointing into `sensor_data`. `MQTT_EVENT_CONNECTED` loops over it to subscribe; `MQTT_EVENT_DATA` loops to match, then falls through to two explicit `strcmp`s for date and time, which are copied as strings. Rejected: putting date and time in the table with a type tag. Two special cases are clearer than a discriminated union for two rows.
