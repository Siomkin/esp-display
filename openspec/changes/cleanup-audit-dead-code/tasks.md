...
- [ ] 1.4 In `Vernon_ST7789T.c`, delete the commented-out `printf` lines and the commented-out MADCTL and COLMOD lines, the duplicate `break;`, and the bare `0x2C` write. Relabel the `0x21` comment "Display Inversion On" (item 15). Verify: builds clean, and the display shows correct colours after boot.
